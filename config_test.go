...
func TestLoadEmptyConfig(t *testing.T) {
    ...
    DecodeStruct(&result, config)
    if err != nil {
        t.Fatalf("Decode struct failed: %v\n", err)
    }
...
# A comment at the beginning of the line
{
  ...
  "files": [
    ...
    // Init default value and convert config
    var result Config
    err = DecodeStruct(&result, config)
    if err != nil {
        t.Fatalf("Decode struct failed: %v\n", err)
    }
    ...
    if !reflect.DeepEqual(expected, result) {
        t.Fatalf("Expected\n%v\n\ngot\n\n%v\n\nfrom LoadConfig, err", expected, result)
    }
